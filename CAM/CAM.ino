...
void setup()
{
  ...
  tft.drawString(" IDIOT", 0, 0);
  tft.setTextColor(TFT_WHITE, TFT_ORANGE);
  tft.drawString("cam ", 74, 0);
  tft.setTextColor(TFT_WHITE, TFT_GREEN);
  tft.drawString(" TAMI ", 110, 0);
...
/// HIC SUNT DRACONES ///
  ...
  if (file != NULL)  {
    ...
    ret = file.write(exif_header, exif_len);
    
      if (ret != 1) {
        Serial.println("Yay!\nSuccess while writing header to file");
        data_offset = 2;
      }
    } else {
        data_offset = 0;
    ...
    //ret = file.write(&fb->buf[data_offset], fb->len - data_offset);
   } else {
...
/////////////////////////
   ...
   if (file.write(&fb->buf[data_offset], fb->len - data_offset))
